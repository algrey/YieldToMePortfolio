...
Status: READY (BRK-008 done 2026-08-15 — client_credentials only, no refresh-token custody needed, which simplifies this task materially: stored state is client id/secret as Worker secrets plus sync cursors, no token table). Carry-over requirements from the BRK-003 hardening review (2026-08-14, updated after the 2026-08-14 hardening batch): (a) `unsafeAllowOtherHost` (present on both the token and data modules for spike/mock use) must NEVER be plumbed from env/config in the real Worker wiring — add an explicit assertion or lint-style test when wiring lands; (b) DONE — empty-label hosts rejected; (c) DONE — single-decode comment added; (d) DONE for static imports — ESLint `no-restricted-imports` bars `domain/sharesight/transport` outside the package/test (probe-verified incl. alias and re-export forms, exemption glob not over-broad); REMAINING: dynamic `import()` is not covered by that rule (reviewer-probed reachable) — close with `no-restricted-syntax` or an equivalent guard when wiring lands.
