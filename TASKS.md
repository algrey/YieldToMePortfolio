...
### IMP-004B — Canonical-security resolution path for brand-new symbols

Status: PENDING OWNER DECISION; blocks UI-only commit of a first-ever import whose symbols have no canonical `securities` rows (e.g. `docs/Example_Portfolio.csv` on an empty account, 57 unresolved candidates).

- Decision required: how canonical securities get created. Options: (a) an owner-facing "verify security" flow that runs a server-side verification path (validates symbol/exchange/currency against provider mappings before publishing a canonical row) — most self-service, needs care in a multi-tenant master; (b) an operator/seed path (curated import of canonical securities for the owner's instruments) — simplest, keeps the master fully operator-controlled; (c) relax the model so owner-private candidates with complete attributes are commit-eligible without a shared master row — largest data-model change, revisits `docs/DATA_MODEL.md` invariants.
- Also fold in (from IMP-004A review follow-ups): a resolve card for `PORTFOLIO_MAPPING_INVALID` (currently the only mapping-class error with no form), and a "Refresh preview" affordance for stale-preview 409s.
- Dependencies: IMP-004A.
- Requirements: IMP-002, IMP-004; `docs/DATA_MODEL.md` securities/master invariants; `docs/MARKET_DATA_STRATEGY.md` provider-mapping rules.
