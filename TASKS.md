...
Status: READY (2026-08-23, owner: "UI-029"). SEQUENCING: after MKT-014 (shared surfaces). Scope per the deferred note: align the ~15 legacy "Price unavailable" DISPLAY strings to the amended AGENTS.md wording ("unavailable") — grep-driven, display-only, no stored data/export changes; update every test pin honestly; also update any doc sentence that quotes the legacy label as current UI copy (append-only where the doc is a dated log). The ARCHITECTURE transition-state note from WLT-001 gets closed out (state the alignment is complete). Original note: AGENTS.md now mandates "unavailable"; ~15 legacy display strings (holding-detail price row, quote-contract preview, import surfaces, etc.) still render "Price unavailable". Display-only alignment task; grep-driven; update the corresponding test pins honestly; no stored data or export changes.
