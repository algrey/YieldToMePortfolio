...
test("generated migration applies cleanly with foreign keys enabled", async () => {
  ...
  assert.deepEqual(indexNames(database, "lot_allocations"), [
    ...
    "lot_allocations_sell_lot_sequence_unique",
  ]);
  ...
  assert.deepEqual(indexNames(database, "tax_lots"), [
    "tax_lots_fifo_idx",
  ...
  assert.deepEqual(foreignKeys(database, "holding_projections"), [
    {
      id: 0,
      seq: 0,
      table: "calculation_runs",
      from: "calculation_run_id",
      to: "id",
      on_update: "no action",
      on_delete: "restrict",
      match: "NONE",
  ...
  assert.deepEqual(foreignKeys(database, "lot_allocations"), [
    {
      id: 0,
      seq: 0,
      table: "calculation_runs",
  ...
  assert.deepEqual(foreignKeys(database, "tax_lots"), [
    {
      id: 0,
      seq: 0,
      table: "calculation_runs",
