...
test("retries by idempotency key and denies cross-owner or cross-portfolio writes", async () => {
  const database = await migratedDatabase();
  const repository = createOwnedLedgerRepository(
    createSqliteSqlClient(database),
  );
  const first = success(await repository.post("user-a", input()));
  const retry = success(await repository.post("user-a", input()));
  assert.equal(retry.idempotent, true);
  assert.equal(retry.transaction.id, first.transaction.id);
  database.exec(
    "UPDATE portfolios SET status = 'archived' WHERE id = 'portfolio-a'",
  );
  const archivedRetry = success(await repository.post("user-a", input()));
