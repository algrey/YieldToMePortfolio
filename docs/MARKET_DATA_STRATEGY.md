...
`domain/sharesight/` (BRK-003+) was originally scoped as a read-only broker-sync source for trades/holdings/payouts, deliberately out of the market-data-provider abstraction above (§7). BRK-012A/BRK-012B narrow that: `listUserInstruments` (`GET /user_instruments.json`) is now ALSO a delayed-price source, seeded into `market_data_providers` as a second row (`id`/`code` = `sharesight`, migration `0044_seed_sharesight_provider.sql`, mirroring `0037`'s `yahoo-compatible` seed). `listUserInstruments`'s single call is treated as the account's complete instrument list only because BRK-017's live probe found it (and `listTrades`/`listPayouts`) unpaginated today — not an assumption baked in silently: `parseItemList` (`domain/sharesight/parse.ts`) fails the whole list closed if a future response ever carries pagination-shaped metadata, per `docs/ARCHITECTURE.md` §8.2.
...
### MKT-018B implementation (guided flow, 2026-08-24)

Built exactly the fallback the spike above recommended — no server-side
fetch of any kind against Intelligent Investor.

- **Coverage read** (`app/price-history-coverage.ts`, `loadOwnedPriceHistoryCoverage`): for the active portfolio's `status = 'held'` securities, ONE batched (never N+1) `price_observations` aggregate query (`COUNT`/`MIN`/`MAX` grouped by `security_id`) computes each security's observation count and first/last observed date, scoped through the SAME owner-visible predicate `app/owned-price-history.ts` uses (deployment rows plus the caller's own user-scoped rows — another owner's user-scoped row on a shared `security_id` is never counted). A correlated subquery resolves each holding's own first-transaction date (`MIN(local_trade_date)` over `posted`/`reversed` transactions) — the same `earliest_trade_date` shape `app/owned-capital-gains.ts` already uses, scoped per-security instead of portfolio-wide.
- **Classification** (`app/price-history-coverage-format.ts`, `classifyPriceHistoryCoverage` — a pure, DB-free function): `zero` (no rows), `partial` (any real gap, an unresolvable first-transaction date, or the B1(b) trailing-staleness rule below), `covered` (reaches across the whole holding period AND is not trailing-stale — excluded from the panel entirely, nothing to do). Gap reporting (`coverageGapSummary`) states only the REAL dates the read returned plus a real calendar-day count where one applies (`app/price-history-chart-geometry.ts`'s `dateDayOffset`, already used by the UI-018 chart's own gap classification) — v1 reports the gap, it never backfills it, per MKT-018's own binding ruling.
- **Review round-1 fixes (2026-08-24, BLOCKING B1/B2, plus folds):**
  - **B1(a):** the original rule (`firstObservationDate <= firstTransactionDate` ⇒ `covered`) missed that the LATEST observation could still be entirely before the holding started — a security whose whole stored history predates the holding read as "covered" with a gap sentence that asserted "no gap," an inference presented as an observation. Fixed: `lastObservationDate < firstTransactionDate` is now checked FIRST and always classifies `partial`, with its own distinct honest sentence ("this history is entirely from BEFORE the holding started").
  - **B1(b), a new binding ruling:** a currently-HELD (not sold-out) security whose last observation is more than `TRAILING_STALENESS_DAYS` (30) before "today" is `partial`, naming the real last-observation date. **Rationale, recorded here per the ruling:** day-to-day price freshness is the LIVE capture pipeline's job (MKT-011A's intraday sweep, Sharesight's 10-minute gate) — this panel exists to catch a BULK CSV-backfill gap, not to nag about ordinary next-day/weekend lag, so 30 days is a deliberately coarse floor that only flags a genuinely stale or abandoned history. **Sold-out securities (shares held as of "today" is ≤ 0) are exempt** from this one rule — there is no ongoing freshness need for a position the owner no longer holds; their pre/during-holding structural gaps ((a) above, and the pre-existing gap-at-the-start check) still classify normally regardless of sold-out status.
  - **B3, review round-2 (BLOCKING):** the FIRST sold-out resolution was a naive buy-minus-sell sum and was SPLIT-BLIND — a real drill (buy 100, split 2:1, sell 150) summed to a false `-50`, wrongly exempting a security that was actually still held 50 shares. Fixed per the reviewer's ruling (no parallel weaker convention): `loadSoldOutPortfolioSecurityIds` (`app/price-history-coverage.ts`) now selects the exact `LedgerQuantityFact` columns (`status IN ('posted', 'reversed')`, every transaction type, not just buy/sell) and DELEGATES the actual quantity derivation to `domain/dividends/shares-held.ts`'s `deriveSharesHeldAtDate(facts, today)` — the SAME pure, already-tested, split- and reversal-aware function `app/dividend-assumptions-actions.ts`'s shares-at-date action uses, anchored to this read's own `today` (the identical local business date the staleness rule itself uses). Still deliberately NOT the full FIFO/cost-basis projection engine `app/owned-holdings.ts` reads from the published `holding_projections` table (that requires a completed, gated calculation run this display-only panel must not depend on) — `deriveSharesHeldAtDate` needs only a signed, split-adjusted quantity sum, which is exactly what this exemption needs. An unresolvable/unparseable derived quantity defaults to "not sold out" (the safer default, so staleness still gets flagged rather than silently exempted).
  - **B2:** the `price_observations` coverage query's `IN (...)` clause bound one parameter per held security — up to `MAX_HELD` (500) plus `userId`, well past D1's documented ~100-bound-parameter-per-statement ceiling. Fixed by chunking (`COVERAGE_READ_CHUNK_SIZE = 50`, mirroring `db/repositories/sharesight-delayed-price-cache.ts`'s identical `CACHE_READ_CHUNK_SIZE` precedent) — every `IN (...)` statement now binds at most ~52 params regardless of portfolio size. Separately, the identities query's `LIMIT MAX_HELD` could silently truncate the download list for a portfolio past that cap; fixed by counting held securities FIRST and failing closed (an honest 503, never a shorter list) both when the count exceeds `MAX_HELD` and when the subsequent list's length doesn't match that count (a genuine concurrent-write race), mirroring `app/owned-holdings.ts`'s identical `heldCount` guard.
  - **Folds:** the coverage aggregate query now joins `securities` and requires `po.currency_code = s.primary_currency_code` — the SAME currency filter `app/owned-price-history.ts` applies per-row, so a row in a currency the chart would never plot cannot count as coverage either. The panel's coverage section is a real `<section aria-labelledby="…">` (was an inert `<div>` with that attribute) matching the parent panel's own precedent.
- **Split for the client bundle (a real build failure this task hit and fixed):** `app/price-history-coverage.ts` (the DB read) and `app/price-history-coverage-format.ts` (pure classification/formatting, no DB import) are deliberately TWO files. The "use client" panel below only ever imports the pure one — `price-history-coverage.ts`'s dynamic `import("./portfolio-actions.ts")` transitively reaches `db/d1-sql-client.ts`'s `cloudflare:workers` import, which broke `vinext build`'s client-environment step (`Rolldown failed to resolve import "cloudflare:workers"`) the first time the panel imported the DB-touching file directly for even one unrelated pure export. Mirrors `app/price-history-chart-geometry.ts`'s pre-existing split from `app/owned-price-history.ts` for the UI-018 chart.
