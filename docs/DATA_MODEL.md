...
**Stored-amount size bound (BUG-014 correction round 3, 2026-09-03).** Every amount column `buildDividendManualRecordImportInsertStatements` writes -- `shares_decimal`, `dividend_per_share_decimal`, `franking_credit_per_share_decimal`, `total_cash_decimal`, `total_franking_decimal` -- is now bounded at the READ path's own `parseDecimal` limits (`domain/calculations/decimal.ts`'s `DECIMAL_LIMITS.inputScale`/`inputDigits`: 24 fractional, 64 total), referenced rather than re-hard-coded, on top of the form-only `DECIMAL_PATTERN`/positivity checks that were previously the only validation. The columns' form validators bound FORM but never SIZE, and no upstream layer bounded a TOTALS-mode amount at all (`computeDividendCashTotal` returns `total_cash_decimal` verbatim, never parsing it), so an over-scale Sharesight payout amount committed cleanly and then threw out of `parseDecimal` on every later `/income` render -- an unrecoverable read-time failure, since these facts are immutable and cannot be edited in place. Rejecting at the write boundary returns `invalid_input`, surfaced by `db/repositories/import-commit.ts` as `mapping_incomplete`; the equivalent 24-place bound on `fx_rate_to_portfolio_decimal` (BRK-010 F3) is unchanged, and keeps its own separate `FX_CONVERSION_SCALE` rationale. See `docs/CSV_IMPORT_SPEC.md`'s BUG-014 correction-round-3 paragraph for the preview-side half of the same bound.
