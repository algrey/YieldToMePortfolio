...
- `total_cash_decimal` (Sharesight's `amount`), `gross_amount_decimal` — both required: unlike `dividend_manual_records`' shares/per-share-XOR-totals amount-mode CHECK, this table only ever stores the totals-mode shape a Sharesight payout carries;
- `total_franking_decimal`, `resident_withholding_tax_decimal`, `non_resident_withholding_tax_decimal` (all nullable);
- `fx_rate_to_portfolio_decimal`/`fx_rate_source` (nullable, paired) — reuses `dividend_manual_records_fx_provenance_check`'s exact pairing rule (`(fx_rate IS NULL) = (fx_rate_source IS NULL)`); the sibling table's second clause (rate implies a recorded currency) is not repeated here because `currency_code` is unconditionally `NOT NULL` on this table;
