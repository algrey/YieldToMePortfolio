...
`value_history_backfill_verified_at`/`value_history_backfill_verified_fingerprint` (nullable, migration `0058_prf_010_value_history_backfill_convergence_marker.sql` — PRF-010): cron-only bookkeeping, never read or written by any user-facing edit path. The hourly value-history backfill (`app/value-history-backfill-service.ts`) was re-running its full `price_observations` `DISTINCT market_date` scan every tick even for a portfolio that had fully converged — measured **52,040** `rows_read` per such tick, ~1.25M/day at 24 ticks, at the owner's real scale (`docs/ARCHITECTURE.md` §9.5). `value_history_backfill_verified_fingerprint` encodes `"<row count>:<min value_date>:<max value_date>:<max computed_at>:<candidate max market_date>"` — a snapshot combining BOTH the STORED side (`portfolio_value_history`'s own row count/min/max `value_date`/max `computed_at` for this portfolio) AND the CANDIDATE side (`MAX(market_date)` over the portfolio's held securities' `price_observations`, under the same `PRICE_SCOPE` predicate `loadCandidateDates` uses) — taken the moment a full check last proved zero candidate dates were missing; `value_history_backfill_verified_at` is that instant. Both halves are required: the stored side alone cannot see a brand-new candidate date that has no prior stored row to invalidate (the routine daily case — a delayed-quote rollup pricing a date for the first time), and `MAX(computed_at)` is needed because row count and min/max `value_date` alone can coincidentally return to their prior values after an interior delete is offset by an unrelated insert elsewhere. `backfillStoredValueHistoryForPortfolio` (`app/historical-portfolio-value.ts`) skips the expensive scan only when the marker is fresh (within `CONVERGENCE_RECHECK_INTERVAL_MS`, 6 hours) AND the current combined fingerprint still matches; the measured cost of that fingerprint check itself is ~2,600 `rows_read` (dominated by the stored-side aggregate visiting every stored row for the portfolio), not free. Deliberately NOT an independently-maintained flag: because the fingerprint is derived live from `portfolio_value_history`'s/`price_observations`' own current state, every existing invalidation path that deletes rows from `portfolio_value_history` invalidates the stored half for free, with no new call site to keep in sync. One known, accepted residual gap: an INTERIOR candidate addition (a backdated price-history import introducing dates older than the current candidate max that were never priced before) moves neither half of the fingerprint — the 6-hour recheck cadence is that residue's primary backstop, not defense in depth on top of an otherwise-complete signal, and the untouched read path independently re-verifies on every page load regardless. See §9.5 (including its dated correction) for the full record.

**BUG-012 fold (2026-09-03):** the encoded fingerprint now has two further fields appended — `portfolio_value_history_unresolvable`'s own row count and `MAX(attempted_at)` for the portfolio (see that table's own entry below). This closes a gap the paragraph above did not anticipate: BUG-012 lets a portfolio with a permanently-unresolvable candidate date converge (excluded from `missingDates`), so without this fold, a later CLEARED mark (a newly-resolvable date) would leave a stale marker claiming "converged" with genuine derivation work now outstanding. Every write path that clears a mark already mutates that table, so this self-invalidates by the same "no new call site" property the rest of this fingerprint relies on.
...
**`transactions_portfolio_source_reference_unique` is a PARTIAL unique index, `WHERE status <> 'reversed'` (`BUG-018`, 2026-09-03; migration `0060_bug_018_reversed_source_reference_partial_index.sql`).** It was a full unique index on `(portfolio_id, source_type, source_reference)` until this fix. `ledger.reverse()` never clears the reversed transaction's `source_reference` (ledger facts are immutable -- correction is reversal, never a rewrite), so under the old full index that key stayed permanently occupied and a reverse-then-re-import of the SAME trade could never land: the commit-time lookup found the reversed row, skipped the re-import, and created no new ledger fact. For a Sharesight sync batch this was unconditional, not merely likely -- a sync row's key (`sharesight-trade:<id>`) is identity-only and never changes, so reversing a synced batch made every one of its trades permanently unsyncable for the life of the account. The partial index now excludes reversed rows from the uniqueness constraint entirely, so a re-import after a reversal is a genuinely NEW posted transaction reusing the freed `source_reference`; the reversed original and its reversal mirror are untouched (immutable). A transaction mid-way through a CHUNKED reversal (`import_batches.status = 'reversing'`, BUG-016) carries no analogous intermediate status of its own -- `ledger.reverse()` flips a transaction's `status` from `'posted'` to `'reversed'` atomically, in the same `client.batch()` as its compensating mirror and inventory update, so within a still-`'reversing'` batch each of its transactions is either fully `'posted'` (not yet reached, still correctly blocking) or fully `'reversed'` (that row's own reversal is complete, correctly no longer blocking) -- never a partial state. The matching `status <> 'reversed'` predicate is carried by every query that asks the same "does this identity already occupy the key" question. **Correction (`BUG-018` review round 2, 2026-09-03):** this paragraph previously listed only three such queries and described that list as complete; it missed a fourth, `db/repositories/ledger.ts`'s `getBySourceReference`, which round 1 left as a FULL match and which is consulted TWICE per write -- as `persist()`'s pre-write check for every `ledger.post`/`ledger.supersede`, and again in `persist()`'s post-`atomic()` error classifier. Left unnarrowed it made the ledger STRICTER than the constraint it anticipates (rejecting a manual re-post after a reversal with `reason: "conflict"`, and mis-classifying unrelated write failures as `conflict` whenever a reversed row shared the reference). The complete, true set is therefore **five** sites, all now identical: this partial unique index itself; `import-commit.ts`'s commit-time trade lookup; `app/import-actions.ts`'s `existingTradeSourceReferences` advisory-suppression query (keeping BUG-013's proven suppression-set-equals-commit-skip-set invariant); `db/repositories/sharesight-sync-state.ts`'s `loadCommittedSharesightRowValues` (BRK-014's value-comparison map, which mirrors the commit predicate by design -- a reversed Sharesight trade now reads as NEW on the next sync); and `db/repositories/ledger.ts`'s `getBySourceReference`. `loadCommittedSharesightWatermarks` is deliberately NOT in this set -- it answers a different question (`status = 'posted'`) and already excluded reversed rows. A consequence of the shape this index now permits, recorded in `docs/ARCHITECTURE.md`'s decision log: a portfolio may legitimately hold a reversed original and a re-imported twin sharing one `source_reference`, so any code that replays transactions (bundle/backup restore) must replay a reversal IMMEDIATELY after the transaction it targets -- see `domain/exports/chain-order.ts`. Dividends need no analogous change: `dividend_manual_records` has no reversed-row concept at all -- a reversal hard-DELETEs the record, freeing its `source_reference` immediately.

**`import_batches_user_file_parser_unique` is likewise a PARTIAL unique index, `WHERE status <> 'reversed'` (`BRK-020`, 2026-09-03; migration `0062_brk_020_reversed_batch_file_key_partial_index.sql`).** This is the batch-identity gap `BUG-018`'s own write-up disclosed as out of scope. The file-level dedup key `(user_id, file_sha256, parser_format, parser_version)` was a full unique index, so `db/repositories/import-staging.ts`'s `startUpload` (`INSERT ... ON CONFLICT ... DO NOTHING`, then a lookup of the colliding row) resolved any upload/sync whose content hash matched a REVERSED batch to that terminal batch: `reused: true`, rows already `commit_status = 'reversed'`, nothing that can be readied or committed again. For a Sharesight sync this was the realistic case, not an edge: `canonicalFetchDigestSource` hashes only the fetched content, so a re-sync after reversing a batch hashes byte-identically whenever nothing else changed in the account, and the owner could never re-sync those trades. CSV's `supersedesBatchId` escape hatch had the same hole for an identical corrected file (`supersedesBatchId` only relaxes the target-portfolio check; the INSERT still collided). A reversed batch is an immutable audit record and is never rewritten to free the key, so the constraint itself is narrowed exactly as `BUG-018` narrowed the transaction identity: a reversed batch drops out of the uniqueness constraint, an identical upload/sync after a reversal INSERTs a fresh batch (a new id, `status = 'uploaded'`, the same `file_sha256`), and any number of reversed generations may share the key with at most one live batch. A batch mid-reversal (`'reversing'`) still occupies the key. `startUpload`'s `ON CONFLICT` target repeats the index predicate verbatim (SQLite matches a conflict target against a partial index only when the `WHERE` matches) and its duplicate-lookup fallback filters `status <> 'reversed'` so it can never hand back a reversed row as the "existing" batch. That `ON CONFLICT` is the only one in the codebase targeting these columns (grepped); `app/portfolio-bundle-service.ts`'s bundle-restore scaffold reads the key with a plain SELECT and INSERTs only when no row exists, and `db/repositories/system-backup.ts` counts every batch with a fingerprint regardless of status, so neither needed a change for correctness. Because neither filters on `status`, SQLite can no longer use this partial index for them: both now seek `import_batches_owner_status_updated_at_idx` on `user_id` alone and filter the fingerprint columns in the VM -- a walk of one owner's own batches on two paths that run once per bundle restore or backup count (measured with `EXPLAIN QUERY PLAN`; accepted deliberately, see `docs/ARCHITECTURE.md`'s `BRK-020` entry for why neither a second covering index nor a `status` predicate on those two queries was added). **Correction (`BRK-020` review round, F1, 2026-09-04):** this paragraph's closing clause above -- "so neither needed a change for correctness" -- remains true for `db/repositories/system-backup.ts`'s unconditional fingerprint count, but not for the OTHER reader it was grouped with: the bundle-restore scaffold's plain SELECT (`findExistingBatch`, `app/portfolio-bundle-service.ts`) DID need a change: this key can now hold several `reversed` rows plus one live row, and a bare `LIMIT 1` with no `ORDER BY` returns whichever matching row the planner visits first -- observed in practice to depend on where each status sorts within `import_batches_owner_status_updated_at_idx`'s `(user_id, status, updated_at)` ordering (e.g. `'failed'` and `'committed'` sort before `'reversed'` and so were returned correctly "by accident," while `'reversing'` sorts after `'reversed'` and was returned WRONG). The query now carries `ORDER BY CASE WHEN status <> 'reversed' THEN 0 ELSE 1 END, updated_at DESC` so the live row wins by construction regardless of physical order; `EXPLAIN QUERY PLAN` shows the same per-owner scan plus a temp B-tree for the sort, which is acceptable at this table's per-owner size. `db/repositories/system-backup.ts`'s unconditional fingerprint count needed no equivalent change -- it counts rows, and does not select or act on any single one of them, so which row a tie resolves to is not observable.

**`failed` decision (`BRK-020` review round, B1, 2026-09-04): `failed` keeps occupying the file key exactly as `reversed` does not -- the index predicate stays `WHERE status <> 'reversed'`, no migration.** `import_batches.status = 'failed'` is written in exactly three places, all in `app/portfolio-bundle-service.ts` (verified by grepping every literal assignment of `'failed'` to this column under `app/` and `db/`): lines ~484, ~545 and ~1100, and all three are reached only for `parser_format = 'portfolio-bundle-json'` (the bundle-restore path). (Corrected 2026-09-04 after the round-3 review: the three writers are the `commitFailure` helper and the two inline failure writers in `app/portfolio-bundle-service.ts` — cite them by name, since the line numbers above moved in the same commit that recorded them; and the guard test's scan now covers `app/`, `db/`, `domain/` and `worker/` plus every `.transitionStatus(` call site's `nextStatus` literal, with a floor of at least the two known call sites, not only `app/` and `db/`.) That path's own reader, `findExistingBatch`, treats ANY non-`committed` row it finds -- `failed` included -- as reusable: it resets `target_portfolio_id` to `NULL` and `status` back to `committing` on the next attempt, so a bundle-restore batch can never get stuck in `failed` the way a `reversed` batch could get stuck occupying the key. `db/repositories/import-staging.ts` (the CSV/Sharesight staging path) never writes `'failed'` to `import_batches.status` today: `persistParsedResult` writes only `invalid` and `parsed`, and the two live `transitionStatus` call sites (`app/import-ready-service.ts`, `app/import-row-exclusion-service.ts`) pass only `ready` and `needs_mapping` respectively -- `isValidTransition`'s table does allow a transition INTO `failed` from most CSV/Sharesight statuses (and `failed: []` out, matching `reversed: []`, i.e. `failed` is terminal), but nothing in the codebase currently calls `transitionStatus` with `nextStatus: "failed"`. So a CSV or Sharesight batch cannot reach `failed` today, and the stuck-key hazard `BRK-020` fixed for `reversed` therefore has no live counterpart for `failed` on that path. Both the portfolio-bundle export and the core system backup exclude `import_batches` from what they carry (verified: neither's own row/table list references it), so a restore can never reinstate a `failed` CSV/Sharesight batch either. If a future writer ever sets `import_batches.status = 'failed'` for a NON-bundle `parser_format` (i.e. actually reaches it via `transitionStatus`), this decision must be revisited: the index predicate `WHERE status <> 'reversed'` on `import_batches_user_file_parser_unique` and the `ON CONFLICT(user_id, file_sha256, parser_format, parser_version) WHERE status <> 'reversed'` clause in `db/repositories/import-staging.ts`'s `startUpload` would both need to move together (to `WHERE status NOT IN ('reversed', 'failed')` or equivalent) or a CSV/Sharesight upload could get stuck in `failed` exactly as `BRK-020` fixed for `reversed`. `tests/brk-020.test.ts` pins the precondition of this decision with a source-scan test asserting every `'failed'` assignment to `import_batches.status` lives in `app/portfolio-bundle-service.ts`, so a new writer for another format fails that test and forces this decision to be revisited rather than silently reproducing the stuck-key hazard.
...
**`source_reference` identity is per-route, not economic, here too (`BUG-013`, 2026-09-02) — the dividend analog of the `transactions` note above.** Cross-batch dividend dedupe (`db/repositories/import-commit.ts`) is an exact-string lookup against the unique index on `(portfolio_id, source_reference)` — it identifies "the same staged row re-committed," never "the same real-world distribution." CSV import and Sharesight sync mint structurally disjoint `source_reference` key spaces for the identical real payout (a sha256 over CSV normalized fields vs. `import-fingerprint:sharesight-payout:<sharesightPortfolioId>:<holdingId>:<paidOnDate>`), so importing the same distribution through both routes posts it twice while satisfying the unique index — there is no schema-level economic-identity constraint (security + payment date + cash total) and none is added by this fix, for the identical reason `BUG-011`'s trade note gives: a genuine same-date repeat distribution must remain insertable. The mitigation is a non-blocking preview-time warning (`DIVIDEND_MATCHES_EXISTING_ENTRY`, `docs/CSV_IMPORT_SPEC.md`), not a stricter identity or constraint. This task ALSO widened the comparison-set query behind that warning (and behind the pre-existing `DIVIDEND_NEAR_EXISTING_ENTRY`) from `import_batch_id IS NULL` (owner-typed only) to every non-superseded row regardless of route — that filter had made every import-sourced record invisible to both checks, which was the confirmed root cause of a silent cross-route double-commit (see `docs/CSV_IMPORT_SPEC.md`'s "Cross-route dividend duplicate warning" entry for the full defect). DIV-016 part C's own reconciliation-candidates query keeps its `import_batch_id IS NULL` scoping unchanged — that is a distinct, correct business rule (only a manually entered fact can be superseded), not the same bug.
...
**Sharesight reconciliation (DIV-016 part C, 2026-08-26): the imported-successor case.** `superseded_by_record_id` is not restricted to owner-facing corrections (Part A) — it also records a SECOND, structurally distinct correction: an existing MANUAL row (`import_batch_id IS NULL`) superseded by a NEWLY IMPORTED row for the same real-world distribution, so a manually entered dividend and its later Sharesight-reported equivalent are never both counted (owner ruling: "if I later synced with sharesight it should not double count"; "sharesight should take precedence from there forward"). Only the row being pointed FROM must be manual, matching `supersede()`'s own guard — the row pointed TO may be either kind. See `docs/CSV_IMPORT_SPEC.md`'s "Sharesight reconciliation (DIV-016 part C)" section for the full matching rule, tolerance decision, ambiguity fail-safe, and reversal-restore mechanics; the short version: `db/repositories/import-commit.ts`'s commit loop sets the guard identically to `supersede()`'s (`import_batch_id IS NULL AND superseded_by_record_id IS NULL`) in the same atomic batch as the new imported row's own insert, and `db/repositories/import-reversal.ts`'s `finalize()` NULLs `superseded_by_record_id` back out for every manual row pointing at one of the reversed batch's own rows — in the same atomic statement set as the DELETE that removes them — so the manual row becomes the head of its lineage again the instant its superseding import is reversed, never silently lost. Because the successor here is import-sourced, Part A's own import-row edit block already prevents a manual re-edit from landing on top of it (`supersede()` rejects any original with `import_batch_id IS NOT NULL`), which is precisely the "Sharesight wins from here forward" precedence the owner asked for — no separate mechanism was needed.
...
**Stored-amount size bound (BUG-014 correction round 3, 2026-09-03; extended to every writer by BUG-022, 2026-09-03).** Every amount column any writer inserts into this table -- `shares_decimal`, `dividend_per_share_decimal`, `franking_credit_per_share_decimal`, `total_cash_decimal`, `total_franking_decimal` -- is bounded at the READ path's own `parseDecimal` limits (`domain/calculations/decimal.ts`'s `DECIMAL_LIMITS.inputScale`/`inputDigits`: 24 fractional, 64 total), referenced rather than re-hard-coded, on top of the form-only `DECIMAL_PATTERN`/positivity checks that were previously the only validation at the two writers BUG-022 fixed. The columns' form validators bound FORM but never SIZE, and no upstream layer bounded a TOTALS-mode amount at all (`computeDividendCashTotal` returns `total_cash_decimal` verbatim, never parsing it), so an over-scale amount committed cleanly and then threw out of `parseDecimal` on every later `/income` render -- an unrecoverable read-time failure, since these facts are immutable and cannot be edited in place. Every writer now shares this ONE bound (`db/repositories/dividends.ts`'s `isWithinReadPathDecimalBounds`):

- `buildDividendManualRecordImportInsertStatements` (BUG-014): the import-commit path (`db/repositories/import-commit.ts`) and `app/portfolio-bundle-service.ts`'s bundle/system-backup restore `wasImported: true` branch (both restore call sites, `~:744`/`~:1932`), which replays an imported record through this same builder.
- `validateManualRecordAmounts`, used by `createDividendManualRecordRepository().create()` (BUG-022): the owner-facing manual-entry dialog (`app/dividend-assumptions-actions.ts`'s `saveDividendEntryWithContext`) AND `app/portfolio-bundle-service.ts`'s bundle/system-backup restore non-`wasImported` (owner-typed) branch (both restore call sites, `~:774`/`~:1980`), which calls the SAME repository `create()` -- fixing the repository function closes both call sites without changing that file.
- `resolveSupersedeAmounts`, used by `createDividendManualRecordRepository().supersede()` (BUG-022): the DIV-016 owner-facing correction/edit path (`saveDividendEntryWithContext`'s edit branch). A carried-forward (unchanged) field from the record being superseded is bounded too, not only a freshly-submitted one.

Rejecting at any write boundary returns `invalid_input`; `db/repositories/import-commit.ts` surfaces it as `mapping_incomplete`, and `saveDividendEntryWithContext` surfaces it as a field-specific 400 (BUG-022: previously a generic "could not be saved" for the two owner-facing writers, now names which field and why, mirroring the pre-existing form-only field errors). The equivalent 24-place SCALE bound on `fx_rate_to_portfolio_decimal` (BRK-010 F3) now also carries the matching 64-digit TOTAL-digit bound (BUG-022) -- the scale-only check previously let an unbounded-length whole-number rate (no fractional part at all) through, past `parseDecimal`'s digit limit at read time; `fx_rate_to_portfolio_decimal` is written only by `buildDividendManualRecordImportInsertStatements`, so this is a single-site fix. See `docs/CSV_IMPORT_SPEC.md`'s BUG-014 correction-round-3 and BUG-022 paragraphs for the preview-side half of these bounds, including the new franking-column preview warnings.
...
- `last_synced_at`, `last_trade_watermark` (nullable text). `BRK-005` set `last_synced_at`'s actual semantics: it moves on successful STAGING, never on commit (accepting a batch is a separate, later, owner-driven step). `last_trade_watermark` remains COMPLETELY UNUSED — `BRK-015` (owner ruling, 2026-09-02) added routine-sync fetch narrowing but deliberately did NOT key it off this column (or off `last_synced_at`): staging a batch and never accepting it is a confirmed-likely path on this account, and either sync-state column only ever advances on staging, so narrowing against it would silently move the fetch window past rows a batch merely staged but never committed. The narrowing watermark instead derives live from `transactions`/`dividend_manual_records` (`db/repositories/sharesight-sync-state.ts`'s `loadCommittedSharesightWatermarks` — TWO independent values, one per stream, never one shared `MAX` across both; see those tables' own entries and `docs/ARCHITECTURE.md` §8.2's BRK-015 review-round entry for the cross-stream hazard a single shared value caused). This column is kept, reserved, in case a future task finds a genuine use for a persisted watermark distinct from derived committed state;
...
### `sharesight_pending_payouts` (BRK-022)

An OBSERVATION table, not a ledger fact — announced-but-unpaid Sharesight payouts. `domain/sharesight-sync/transform.ts`'s `isFutureUnconfirmedPayout` used to skip every future-dated null-id payout outright (`SHARESIGHT_PAYOUT_UNCONFIRMED`), throwing the announcement away; slice 2 of BRK-022 instead upserts it here, so a distribution the owner's broker has declared but not yet paid is visible before it commits. Owner ruling (2026-09-04): no staging/review step — upserted DIRECTLY at sync time (an announcement is a provider observation that refreshes or withdraws on every sync, unlike a CSV import row, which is staged/previewed/reversible before it ever touches a ledger table).

`source_reference` on THIS table stores the BARE payout identity key `BRK-005C` mints (`sharesight-payout:<portfolio>:<holding>:<paidOn>`), independent of Sharesight's confirmed/null id. **Review round correction (BRK-022 slice 2, B2, BLOCKING, 2026-09-04): this is NOT byte-equal to the LATER paid `dividend_manual_records.source_reference` under "the same" identity, as an earlier version of this paragraph claimed.** `db/repositories/import-commit.ts`'s commit-time identity scheme wraps every staged row's own fingerprint (the bare key, for a Sharesight row) in an `import-fingerprint:` prefix shared by every import source, CSV or Sharesight alike — a committed row's `source_reference` is therefore `import-fingerprint:<the same bare key>`, not the bare key itself. The two are related by `domain/imports/committed-source-reference.ts`'s `committedSourceReferenceForFingerprint(fingerprint)`, the single exported function that mints that prefix — the "paid overrides pending" rule slice 3 (read path) relies on comparing `committedSourceReferenceForFingerprint(pending.sourceReference)` against the committed row's own `source_reference`, never the bare key against the prefixed one directly. Once a committed row exists under that derived reference, the read path suppresses the pending row entirely, never double-counting. This table itself enforces nothing about that suppression — it is a pure sync-time cache of what Sharesight currently says is outstanding for one portfolio, unique per `(user_id, portfolio_id, source_reference)`. **Review round correction (BRK-022, B1, BLOCKING, 2026-09-04):** the key was originally `(portfolio_id, source_reference)` only — `user_id` was not part of the unique index, so `upsertObserved`'s `ON CONFLICT (portfolio_id, source_reference) DO UPDATE` could match another owner's row (a colliding `source_reference` under the same `portfolio_id` argument) without the composite `(portfolio_id, user_id)` FK ever being evaluated on the UPDATE path, silently overwriting that owner's values. `user_id` is now part of both the unique index and the `ON CONFLICT` target, so a cross-owner call always falls through to the INSERT path instead, where the FK rejects it as a typed `atomic_failure`.

- `id`, `user_id`, `portfolio_id` (composite FK to `portfolios(id, user_id)`, `ON DELETE RESTRICT`, mirroring `dividend_manual_records`);
- `portfolio_security_id` (nullable, composite FK to `portfolio_securities(id, user_id, portfolio_id)`, `ON DELETE RESTRICT`) — NULLABLE, unlike `dividend_manual_records` where it is required: an announced payout can arrive before the sync's own security-resolution step (slice 2, mirroring `loadResolvedPortfolioInstrumentCurrencies`'s `security_identifiers`/`source_symbol` lookup) finds an owned `portfolio_securities` row for it. `null` means "not yet resolvable to an owned security", not an error; the read path (slice 3) skips a null-security row rather than guessing;
- `source_reference` (the payout identity key above), `sharesight_holding_id`, `sharesight_instrument_id` (nullable), `sharesight_payout_id` (nullable — Sharesight's own confirmed id, present once the broker later confirms the same distribution without the app ever having re-fetched under a new identity), `symbol`, `market_code`;
- `currency_code` (FK `currencies(code)`, `ON DELETE RESTRICT`), `payment_date`, `ex_date` (nullable — the window-coverage predicate below depends on this being honestly absent when Sharesight did not supply it, never guessed);
- `total_cash_decimal` (Sharesight's `amount`), `gross_amount_decimal` — both required, and validated as NON-NEGATIVE decimal strings (review round correction, F3, 2026-09-04: originally required strictly positive, but Sharesight can genuinely announce a `"0"` payout, and this table's write is validated WHOLESALE — a single rejected zero-amount row would have blocked recording AND withdrawal for the entire portfolio, not just that one payout): unlike `dividend_manual_records`' shares/per-share-XOR-totals amount-mode CHECK, this table only ever stores the totals-mode shape a Sharesight payout carries;
- `total_franking_decimal`, `resident_withholding_tax_decimal`, `non_resident_withholding_tax_decimal` (all nullable);
- `fx_rate_to_portfolio_decimal`/`fx_rate_source` (nullable, paired) — reuses `dividend_manual_records_fx_provenance_check`'s exact pairing rule (`(fx_rate IS NULL) = (fx_rate_source IS NULL)`); the sibling table's second clause (rate implies a recorded currency) is not repeated here because `currency_code` is unconditionally `NOT NULL` on this table. F5: unlike `dividend_manual_records`, where `currency_code` is populated only when the payout is foreign to its security, `currency_code` here is ALWAYS populated (it is simply the payout's own reported currency) — a reader must compare it against the resolved security's own currency to tell native from foreign, never treat its mere presence as "foreign";
- `first_observed_at`/`last_observed_at` (NOT NULL ISO instants) — let the repository distinguish "seen once, still current" from "refreshed on every subsequent sync"; `first_observed_at` is set once and never touched again;
- `withdrawn_at` (nullable) — set when the most recent COVERING sync no longer observed this `source_reference`; re-observing clears it back to `NULL`;
- `created_at`, `updated_at`.

All decimal columns are validated decimal strings, bounded by `isWithinReadPathDecimalBounds` (BUG-014/BUG-022's shared read-path bound) at the repository write boundary — no DB-level decimal-format `CHECK`, matching every other financial table in this schema. No float arithmetic anywhere in the write or read path.

FKs: `(portfolio_id, user_id) → portfolios(id, user_id)`, `(portfolio_security_id, user_id, portfolio_id) → portfolio_securities(id, user_id, portfolio_id)`, `currency_code → currencies(code)`, all `ON DELETE RESTRICT`. Unique `(user_id, portfolio_id, source_reference)` — the table's natural key; an upsert overwrites value columns in place rather than accumulating history. Indexes: `(user_id, portfolio_id, withdrawn_at)` (the active-rows lookup `listActive` and the withdrawal sweep both use) and `(user_id, portfolio_id, portfolio_security_id)`. CREATE-only migration (`drizzle/0063_brk_022_sharesight_pending_payouts.sql`) with hand-appended `account_purge_lock_*` triggers in the SAME migration, following `sharesight_delayed_prices`'s (`0045`) established precedent exactly (no rebuild, no drop-the-trigger hazard). Classified `owned` (ownerColumn `user_id`) for export/purge (`ACCOUNT_EXPORT_TABLE_CLASSIFICATIONS`/`PURGE_TABLES_IN_FK_ORDER` in `db/repositories/account-lifecycle.ts`) — purged before `portfolio_securities`/`portfolios` (it references both), unlike `sharesight_sync_state`, which only references `portfolios`.

`db/repositories/sharesight-pending-payouts.ts` provides the repository this slice reserves: `upsertObserved` (keyed by `(user_id, portfolio_id, source_reference)` via a single `INSERT ... ON CONFLICT DO UPDATE` per row, chunked at `REFERENCE_CHUNK_SIZE` (50) statements per `client.batch()` call — observations are idempotent, so a partially applied multi-chunk call is safe, the next sync simply re-observes; input rows are also de-duplicated by `sourceReference` before any statement is built, last occurrence wins, so a duplicate inside one call neither double-writes nor inflates the `inserted` count — inserting sets `first_observed_at = last_observed_at = now`; a conflict refreshes every value column, advances `last_observed_at`/`updated_at`, clears `withdrawn_at`, and — by omission from the `DO UPDATE SET` list — leaves `id`/`first_observed_at`/`created_at` exactly as they were), `markWithdrawnNotObserved` (withdraws every active row for one portfolio whose `source_reference` was not in the sync's observed set AND that the sync's own fetch window actually covered — a `SharesightStreamWindow` `"full"` window covers every active row, a `"narrowed"` one only a row with a non-null `ex_date` on or after `window.sinceDate`, so a null-`ex_date` row is only ever withdrawn by a full window), and `listActive` (owner+portfolio-scoped, `withdrawn_at IS NULL`, ordered by `payment_date, symbol`). Every query is `user_id`-scoped; decimal validation failures return a typed `{ ok: false, reason: "invalid_input", field }` result rather than throwing, and nothing in the batch is written if any row fails validation.

**Read-path suppression (BRK-022 slice 3).** `app/owned-dividend-history.ts` loads every active row (`listActive`) and, BEFORE any row reaches `deriveDividendHistoryForSecurity`, skips it when either: (a) **identity** — a currently-committed `dividend_manual_records` row (`superseded_by_record_id IS NULL`) has `source_reference === committedSourceReferenceForFingerprint(pending.sourceReference)` (built from the ALREADY-LOADED, portfolio-wide `manualRecords` array every `manualBySecurity` row is also built from — sourced from `createDividendManualRecordRepository.list`, whose only filter is `superseded_by_record_id IS NULL`. **F1 correction (accuracy fix):** this is NOT "the same predicate `loadCommittedSharesightRowValues` uses" — that function's own `payoutRows` query has no supersession filter at all (it reads every `dividend_manual_records` row matching the source-reference prefix, unfiltered), so the two functions do not in fact share one predicate. The reason a reversed record cannot suppress a pending row is structural, not predicate-matching: `db/repositories/import-reversal.ts`'s reversal statements HARD-DELETE every `dividend_manual_records` row for the reversed batch, so a reversed record is simply absent from `manualRecords` by the time this function runs); or (b) **proximity** — no identity match, but a currently-committed record for the SAME `portfolio_security_id` (any source) has a `payment_date` within `domain/dividends/history.ts`'s `PROXIMITY_WINDOW_DAYS` (7) of the pending row's own, amount-agnostic on purpose (a DRP/fee-adjusted receipt must still suppress the announcement). A row with `portfolio_security_id IS NULL`, or resolving to a security outside this load's held/composition set, is counted `pendingUnresolved` (`OwnedDividendHistory.pendingPayoutCounts`) rather than crashing or vanishing silently. A row that survives suppression is fed into the derivation as a `DividendManualRecordFact` with `announcedUnpaid: true`, `importBatchId: null` — landing in the "imported" tier by that flag alone, since it is provider evidence the owner never personally typed. See `docs/CALCULATIONS.md` §11's BRK-022 paragraph for the resulting FY-total/forecast/list rules.

**Read-path completeness and cap (F5/F7 correction round, 2026-09-05).** The pre-fix early-return for a portfolio with NO held securities at all (`identities.length === 0`) skipped `listActive` entirely, so a pending payout belonging to an entirely-unheld portfolio was never even read, let alone disclosed. It now still calls `listActive` (unless narrowed to one security — that consumer never reads `pendingPayoutCounts`) and counts every returned row `pendingUnresolved` — correct by construction, since with zero held identities no row can possibly resolve to one. Separately, `listActive` gained an optional `limit` parameter (a plain SQL `LIMIT`); the loader passes `MAX_PENDING_PAYOUTS_PER_PORTFOLIO + 1` (500, `app/owned-dividend-history.ts`, next to its sibling `MAX_*` constants) on both call sites so it can detect and disclose truncation (`OwnedDividendHistory.pendingTruncated`) rather than reading the whole table unbounded.
...
1. `app/historical-portfolio-value.ts`'s `loadHistoricalPortfolioValueSeries` -- on every Overview graph read, derives and persists up to `MAX_DERIVE_DATES_PER_READ` (400) of the newest candidate dates NOT yet stored (this is both the initial backfill and every later day's incremental append -- ONE mechanism, not two). A fully-covered range skips derivation entirely and serves stored rows directly. This is the ONLY reader of this table -- Multi-Year's FY-end lookups never read it (see below). **BUG-010 correction (2026-09-01, not edited in place per this file's record convention): `MAX_DERIVE_DATES_PER_READ` is now 10, not 400 -- 400 was ~104ms of measured Worker CPU against a Workers Free allowance of 10ms per invocation, which turned any wiped series into a permanent Error 1102 loop (the read was killed before it could persist its slice, so the next read repeated identically). "ONLY reader" is also superseded by write path (1b) below.**
   1b. `app/value-history-backfill-service.ts`'s `sweepValueHistoryBackfill` (BUG-010) -- the hourly cron's bounded rebuild, so a wiped series recovers without the owner repeatedly loading the Overview. Calls the SAME `app/historical-portfolio-value.ts` mechanism (`backfillStoredValueHistoryForPortfolio`), with its own per-TICK date budget (`CRON_MAX_BACKFILL_DATES_PER_TICK`, a total across every portfolio the tick touches, sized against the same Workers Free 10ms -- Cloudflare gives the scheduled handler no extra CPU on that plan) and slicing the OLDEST missing candidate dates while the read path slices the newest. Owner scoping comes from `portfolios.user_id`'s own authoritative column, never a client-supplied id. No new formula, no new column, and the same honesty invariant: an unresolvable date is left absent, never stored.
...
**BUG-012 correction (2026-09-03, not edited in place per this file's record convention):** a candidate date the derivation genuinely could not resolve was never stored here, so it stayed "missing" forever and was re-attempted on every read/cron call -- a contiguous run of such dates at least as long as one call's bound could permanently pin that call's bounded slice, starving every candidate date behind it. That hazard is now closed by a sibling table -- see `portfolio_value_history_unresolvable` immediately below.

### `portfolio_value_history_unresolvable` (BUG-012)

A sibling, owner-scoped record of "attempted, genuinely unresolvable" for a `portfolio_value_history` candidate date -- one row per (portfolio, date) the derivation attempted and could NOT resolve a value for (nothing held, or every held security missing a price/FX within tolerance). Not a placeholder inside `portfolio_value_history` itself: that table's `value_decimal`/`completeness`/`held_security_count`/`priced_security_count` columns are all `NOT NULL` with CHECK constraints tying `priced_security_count` to `held_security_count`, and SQLite cannot relax an existing `NOT NULL` column via `ALTER TABLE` without a full table rebuild (drop + recreate + copy) -- a cost this task's additive, no-rebuild schema change avoids. A sibling table with the identical owner-scoped unique key (`portfolio_id`, `value_date`) achieves the same result with two plain `CREATE TABLE`d columns instead.

- `id`, `user_id`, `portfolio_id`, `value_date` (`YYYY-MM-DD`);
- `reason` (`'no_holdings' | 'no_priceable_security'`, CHECK-enforced) -- `'no_holdings'` when `held_security_count` was 0 that day (e.g. a candidate date before this portfolio's first trade in a currently-held security, or after a full sale with no re-buy); `'no_priceable_security'` when securities were held but none resolved a price/FX within tolerance. Diagnostic categorisation only -- both reasons are cleared identically by every invalidation path below;
- `attempted_at` -- when the derivation last determined this date was unresolvable; provenance/audit only, never displayed;
- `fingerprint` (nullable) -- a diagnostic-only snapshot of the facts behind this determination (`held=<n>;priced=<n>`) for a human investigating why a date is marked; unlike PRF-010's convergence fingerprint below, nothing in this codebase compares it for equality.

Natural key `(portfolio_id, value_date)` (unique index), same shape as `portfolio_value_history`'s own. FK: composite `(portfolio_id, user_id) → portfolios(id, user_id)` `ON DELETE RESTRICT`, identical to `portfolio_value_history`'s own FK. Index on `(user_id, portfolio_id, value_date)`. CREATE-only migration (`drizzle/0059_bug_012_value_history_unresolvable.sql`) with hand-appended `account_purge_lock_*` triggers in the SAME migration, following `portfolio_value_history`'s (`0054`) own precedent exactly (`tests/bug-012.test.ts` pins trigger existence and firing). Classified `owned` (ownerColumn `user_id`) for export/purge, positioned in the purge FK-order list next to `portfolio_value_history` (references only `portfolios`, no table references it).

**Read path:** `app/historical-portfolio-value.ts`'s `resolveValueHistorySeries` (the shared core behind both `loadHistoricalPortfolioValueSeries` and `backfillStoredValueHistoryForPortfolio`) loads this table alongside the existing stored-row read and excludes any already-marked date from `missingDates` -- such a date can never occupy a slot in a bounded derive slice again, closing the stall hazard by construction. A date this call's OWN derivation attempts and still cannot resolve is marked immediately after, via `db/repositories/portfolio-value-history.ts`'s `recordUnresolvableValueHistoryDates`. This is a CACHED "not resolvable as of the last attempt" fact, never a permanent write-off.

**Invalidation (load-bearing correctness property):** every write path that already invalidates `portfolio_value_history` for a date range ALSO clears the matching rows here, in the same atomic unit as its existing DELETE -- a date this portfolio had marked unresolvable is exactly the shape of stale fact each of those paths already exists to correct:

- `db/repositories/ledger.ts`'s `persist`/`buildLedgerPostingStatements` and `db/repositories/import-commit.ts`'s `finalize` push `unresolvableValueHistoryClearFromDateStatement` (a `value_date >= fromDate` ranged DELETE) into the SAME atomic batch as their existing `valueHistoryInvalidationFromDateStatement` call.
- `app/price-upload-service.ts`'s `confirmSinglePriceUpload`/`confirmBackupPriceUpload`, via `invalidateStoredValueHistoryForSecurity` → `deleteStoredValueHistoryInRangeForOwnedSecurity`, now issue BOTH the `portfolio_value_history` DELETE and this table's DELETE as one atomic `client.batch()` call per affected portfolio.
- `db/repositories/market-data-refresh.ts`'s Yahoo-compatible rollup and `db/repositories/intraday-price-capture.ts`'s MKT-011A capture, via `buildValueHistoryInvalidationStatementsForSecurities`, now emit a paired DELETE statement for this table alongside each `portfolio_value_history` DELETE statement they already build, for the caller's own atomic batch.
- `db/repositories/sharesight-price-refresh.ts`'s `upsertSharesightPriceObservations` (the hourly Sharesight refresh cron AND the MKT-015 prior-day gate backfill) appends the same paired DELETEs to the SAME atomic batch as the chunk's price writes — via `buildOwnerValueHistoryInvalidationStatementGroups`, the USER-SCOPED sibling builder. A Sharesight observation is written `access_scope = 'user'` / `scope_user_id` = the fetching owner, and the read path admits a user-scoped row only for its own owner, so only that owner's own portfolios are resolved; targets are grouped per portfolio (2 statements per affected portfolio, over the EXACT SET of dates the chunk wrote for the securities that portfolio holds — `value_date IN (?, …)`) and any remainder past the atomic batch's room under D1's 100-statement ceiling (less a deliberate 10-statement margin) is issued in follow-on `batch()` calls immediately after it, never dropped. **BUG-012 round-4 B1 correction to the sentence this replaces, which said `[MIN, MAX]` of the chunk's dates:** a folded span deleted every stored row and mark BETWEEN two written dates, which for a portfolio holding one live and one long-halted instrument is months wide and re-done hourly; it also cleared marks for dates the write could not have made resolvable, breaking this section's own rule that a mark is cleared only by a write that can make that date newly resolvable (the value-history read path's `priceToleranceDays` is 0, so a price for date D bears on D alone). BUG-012 round-3 B1: the round-2 implementation instead passed a 20-row cap into the cross-owner builder, which silently truncated — a single portfolio holding 25 repriced securities kept 5 marks and 5 stale rows on EVERY hourly run, and unrelated owners' rows could consume the cap entirely.

**PRF-010 fold:** the convergence-marker fingerprint (`portfolios.value_history_backfill_verified_fingerprint`, described above) now also folds in this table's own row count and `MAX(attempted_at)` for the portfolio -- a write path that clears a mark here (a newly-resolvable date) changes that snapshot, so a stale marker cannot claim "converged" over a portfolio whose unresolvable set just shrank. Because every clearing write path above already mutates this table, the fingerprint self-invalidates for free, with no sixth call site to remember. One consequence, correcting the PRF-010 record above: a portfolio with a permanently-unresolvable candidate date can NOW satisfy `missingDates === 0` and be marked converged, once every remaining candidate date is either stored or marked unresolvable -- this is BUG-012's intended fix, not a regression of PRF-010's marker gate.

**BUG-012 correction round, F2 (2026-09-03): an FX-only gap is deliberately NEVER a row in this table.** Review found the original round marked a date `'no_priceable_security'` (and left it permanently unresolved) even when the sole cause was a missing `fx_rate_observations` row for an otherwise-priced foreign-currency security -- no FX write path invalidated this table (only `kind === 'price'` writes did), so such a mark was never cleared by the FX rate's own later arrival. Orchestrator ruling: `app/historical-portfolio-value.ts`'s `computeUnresolvedReason` now computes a THIRD, in-memory-only reason, `'no_fx_rate'` (never a CHECK-constraint value, never written here), whenever `domain/snapshots/historical-portfolio-value.ts`'s `valuePointAtDate` reports `fxOnlyGap: true` -- every held security had a resolvable price but no usable FX rate for the date. A date classified this way is excluded from the `recordUnresolvableValueHistoryDates` write entirely, so it keeps the PRE-BUG-012 behaviour of being re-attempted on every subsequent read/cron call until the FX rate arrives -- never written off. **Disclosed residual, not a proof:** an FX-caused CONTIGUOUS run of dates at least as long as one call's bound can still pin that call's bounded slice exactly as BUG-010 originally described (an FX ingestion outage spanning more dates than one read/tick derives), mitigated the same way BUG-010 mitigated the general case -- the read sweeps newest, the cron sweeps oldest, so a run pinning one end does not block the other. `tests/bug-012.test.ts` proves both halves: an FX-only gap never gets a row here, and the FX rate's later arrival (with no `portfolio_value_history` mutation, since `market-data-refresh.ts`'s invalidation is still `kind === 'price'`-scoped) is naturally re-attempted on the next read because nothing was ever written off.

**BUG-012 round-3 correction, FU1 (2026-09-03): the F2 rule above is widened from "only" to "any" FX-caused component.** The round-2 predicate (`fxOnlyGap`) required EVERY held security's failure to be FX-caused, so a MIXED date — one unpriced base-currency security plus one otherwise-priced foreign security with no usable FX rate — was still written here as `'no_priceable_security'`. That mark could never be cleared by the FX rate's own arrival (no FX write path touches this table), and a later price for the unpriced security would have cleared a mark on a date that was still FX-blocked. The predicate is renamed `fxMissingComponent` and now means "at least one held security's missing cause was FX": any such component suppresses the row entirely, so the date keeps the pre-BUG-012 retry-on-every-read behaviour. It is still computed only for a fully unresolved point (`valueDecimal === null`), so a PARTIAL point never carries the key and the stored/derived structural parity `tests/hist-002.test.ts` pins is unchanged.
...
CREATE TABLE import_batches (
  ...
  -- BRK-020: (user_id, file_sha256, parser_format, parser_version) is a
  -- PARTIAL unique index, not a plain UNIQUE column list -- see
  -- `import_batches_user_file_parser_unique` below and the note under
  -- the transactions section above. A reversed batch no longer occupies
  -- the file key.
  UNIQUE (id, user_id)
);
CREATE UNIQUE INDEX import_batches_user_file_parser_unique
  ON import_batches(user_id, file_sha256, parser_format, parser_version)
