...
A non-zero security contributes to both `InvestedValue` and `CoveredOpenBasis` only when its home market value and home basis are both available; value-only and basis-only positions are excluded from both aligned amounts and listed in coverage. Converted non-zero cash is tracked separately. Coverage reports total, non-zero, and zero component counts. Because the status and label are produced from the same coverage computation, an incomplete result cannot serialize with the complete discriminator.
