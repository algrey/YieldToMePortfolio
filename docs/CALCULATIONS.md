...
There is no such thing as a negative taxable capital gain: if losses exceed total gains for the year, both "remaining after loss" buckets fully absorb to 0, the net capital gain estimate is 0, and the excess is disclosed separately as an unabsorbed-loss figure. **Loss carry-forward across financial years is OUT OF SCOPE for v1** — every FY is reported standalone, with an explicit standing note (`CGT_CARRY_FORWARD_OUT_OF_SCOPE_NOTE`) wherever an unabsorbed loss appears, rather than silently implying the excess applies to any other year. Each total also echoes a per-figure method label (`CGT_METHOD_LABELS`) so a consumer never has to infer the ordering rule from the numbers alone.
