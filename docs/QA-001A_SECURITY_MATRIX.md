...
- 5 pages, matching §1's route count at the time of this DEP-001
  verification — §1 has since grown to 25 API route handlers with IMP-004A's
  addition of `POST /api/import/preview/:batchId/ready`), the full automated
  suite (290 passed,
...
- `./node_modules/.bin/vinext build` — succeeds, 24 API route handlers + 5 pages built (matched §1's route count as of this QA-001A verification; §1 has since grown to 25 API route handlers with IMP-004A's addition of `POST /api/import/preview/:batchId/ready`)
