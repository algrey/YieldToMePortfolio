...
- **API route PATHS** — one per `app/api/**/route.ts` file:
  `find app/api -name route.ts | wc -l` → **30** as of UI-006C (29 at
  UI-006B time, +1 new path this task:
...
GET/POST = <factory>(...)` form, e.g. the ledger/import-ready routes) per
  ...
  (`POST`/`DELETE`), so the current 30 paths expose **34** handlers in
  total (33 at UI-006B time + this task's 1: the new refresh route's
  single `POST`; 28 at QA-001A time + UI-006B's 5: dividend-assumptions
  POST, dividend-entries POST/DELETE, dividend-fy-overrides POST,
...
or handler), so the current, single source of truth is **30 API route
paths exposing 34 exported HTTP-method handlers, plus 10 pages**.
...
and CGT-001A's `app/owned-capital-gains.ts` (CGT-001B's own new file,
`app/components/capital-gains-screen.tsx`, is a `.tsx` component under
`app/components/`, not the app root, so it was never a candidate for this
match either) do not match this `*route.ts` suffix pattern, so none of
them add a new non-route match). 30 real route files + 5 non-route modules
= 35, the count
this task's verification run of `vinext check` reports (unchanged from
UI-006C time, since CGT-001B added a page, not a route file; 30 at QA-001A
time, 25 + 5).
