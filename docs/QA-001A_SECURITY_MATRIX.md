...
- **API route PATHS** — one per `app/api/**/route.ts` file:
  `find app/api -name route.ts | wc -l` → **29** as of UI-006B (25 at
  QA-001A time, +4 new paths this task: `dividend-assumptions`,
...
GET/POST = <factory>(...)` form, e.g. the ledger/import-ready routes) per
  ...
  2 (`PATCH`/`DELETE`), and this task's new
  `/api/portfolios/:portfolioId/dividend-entries` exports 2
  (`POST`/`DELETE`), so the current 29 paths expose **33** handlers in
  total (28 at QA-001A time + this task's 5: dividend-assumptions POST,
...
→ **8** as of UI-006B (7 at UI-006A time, +1:
...
API paths exposing 5 new handlers (see above), so the current, single
source of truth is **29 API route paths exposing 33 exported HTTP-method
handlers, plus 8 pages**.
...
real `app/api/**/route.ts` handlers, not routes themselves; UI-006B's new
action module, `app/dividend-assumptions-actions.ts`, does not match this
`*route.ts` suffix pattern, so it adds no new non-route match). 29 real
route files + 5 non-route modules = 34, the count this task's verification
run of `vinext check` reports (up from the 30 recorded at QA-001A time,
25 + 5).
