...
| `/api/portfolios/:portfolioId/dividend-franking-override` (BRK-011, `saveDividendFrankingOverrideAction`)                                               | POST          | `getAuthenticatedSqlContext(portfolioId)` → `context.userId`; `createDividendImportFrankingOverrideRepository(...).save` re-verifies the owned portfolio/security AND that the target record is an IMPORTED (`import_batch_id IS NOT NULL`) row belonging to that owner -- an owner-typed manual record is rejected (404), never mutated through this action                                                                                                                                                                                                                                                                                                                                                                                                                                               | Yes (new in this task, `app/api/portfolios/[portfolioId]/dividend-franking-override/route.ts`)                               | `tests/ui-006b.test.ts` "UI-006B: every mutation handler in the new dividend routes rejects cross-site requests before reading the body"; `tests/brk-011.test.ts` "BRK-011 action: saveDividendFrankingOverrideWithContext denies a cross-owner portfolioSecurityId/record and validates input"; "BRK-011 repository: save() rejects an owner-typed (non-imported) manual record as not_found"; "BRK-011 repository: save() rejects a cross-owner imported record as not_found -- never trusts a client-supplied id"                                                                                                                                                                                                                                                                                                                                       |
...
| `/api/portfolios/:portfolioId/securities/:portfolioSecurityId/price-history` (UI-018, `priceHistoryAction` -> `loadOwnedPriceHistory`)                  | GET           | `getAuthenticatedSqlContext(portfolioId)` re-verifies `portfolios WHERE id = ? AND user_id = ?`, then `loadOwnedPriceHistory` separately re-verifies `portfolio_securities WHERE id = ? AND user_id = ? AND portfolio_id = ?` before reading any `price_observations` row; the row query itself is further scoped to that security plus the owner-visible predicate (`access_scope = 'deployment'` OR the caller's own `access_scope = 'user' AND scope_user_id = ?`) shared with `app/owned-holdings.ts`                                                                                                                                                                                                                                                                                                  | N/A (read, new in this task, `app/api/portfolios/[portfolioId]/securities/[portfolioSecurityId]/price-history/route.ts`)     | `tests/ui-018.test.ts` "UI-018: loadOwnedPriceHistory denies a cross-owner portfolioId"; "UI-018: loadOwnedPriceHistory denies a portfolioSecurityId belonging to another user"; "UI-018 (review round-1 fix, F3): the owner-visible scope predicate is pinned -- another user's user-scoped observation on THIS security never appears"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `/api/market-data/price-uploads` (MKT-008, `listPriceUploadsAction`, "Historical Data" section)                                                         | GET           | `getAuthenticatedSqlContext()` → `context.userId`; `listOwnedPriceUploads`/`listPriceUploadBatches` filter `WHERE user_id = ?`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | N/A (read)                                                                                                                   | `tests/mkt-008.test.ts` "MKT-008 listing: past uploads are owner-scoped and never leak across owners"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `/api/market-data/price-uploads/export` (MKT-008, `exportPriceHistoryAction`)                                                                           | GET           | `getAuthenticatedSqlContext()` → `context.userId`; `loadOwnerPriceExportRows` filters `WHERE access_scope = 'user' AND scope_user_id = ?` -- deployment-scoped rows are never the owner's to export                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | N/A (read)                                                                                                                   | `tests/mkt-008.test.ts` "MKT-008 export scope: only the owner's own user-scoped rows export -- never another owner's, never deployment-scoped rows"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `/api/market-data/price-uploads/preview` (MKT-008, `previewSinglePriceUploadAction`)                                                                    | POST          | `getAuthenticatedSqlContext()` → `context.userId`; ticker resolution (`loadSameUserSecurityEvidenceForTicker`) is scoped `WHERE ps.user_id = ?`, never trusting a client-supplied security id; never writes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Yes (new, `app/api/market-data/price-uploads/preview/route.ts`)                                                              | `tests/mkt-008.test.ts` "MKT-008: every mutating price-upload route calls rejectCrossSiteMutation before any other work"; "MKT-008 preview: a happy-path CSV matches the owner's security and reports rowCount/dateRange/samples"; "MKT-008 preview: no security held for the ticker is an honest error naming the ticker (never auto-created)"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `/api/market-data/price-uploads/confirm` (MKT-008, `confirmSinglePriceUploadAction`)                                                                    | POST          | Same context/scope as the preview route above; writes `price_observations`/`price_upload_batches` rows keyed to `context.userId` only, natural-key idempotent upsert (never a duplicate)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Yes (new, `app/api/market-data/price-uploads/confirm/route.ts`)                                                              | `tests/mkt-008.test.ts` "MKT-008: every mutating price-upload route calls rejectCrossSiteMutation before any other work"; "MKT-008 confirm: writes owner-import price_observations with the midnight-Sydney observation_at and guard-creates the provider mapping"; "MKT-008 idempotent overlay: importing the SAME file twice (same fixed now) leaves byte-identical price_observations for that security, attribution staying with the batch that CREATED the rows"                                                                                                                                                                                                                                                                                                                                                                                      |
| `/api/market-data/price-uploads/backup/preview` (MKT-008, `previewBackupPriceUploadAction`)                                                             | POST          | `getAuthenticatedSqlContext()` → `context.userId`; per-row resolution is the SAME owner-scoped evidence query as the single-upload preview above; never writes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Yes (new, `app/api/market-data/price-uploads/backup/preview/route.ts`)                                                       | `tests/mkt-008.test.ts` "MKT-008: every mutating price-upload route calls rejectCrossSiteMutation before any other work"; "MKT-008 export -> wipe -> import-backup: a full round trip is lossless and preserves each row's ORIGINAL provider (never relabelled)"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `/api/market-data/price-uploads/backup/confirm` (MKT-008, `confirmBackupPriceUploadAction`)                                                             | POST          | Same context/scope as the backup-preview route above; every written row is scoped `access_scope = 'user', scope_user_id = context.userId`, and preserves each row's ORIGINAL `provider_id` (never relabels a Sharesight-sourced row as owner-uploaded)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Yes (new, `app/api/market-data/price-uploads/backup/confirm/route.ts`)                                                       | `tests/mkt-008.test.ts` "MKT-008: every mutating price-upload route calls rejectCrossSiteMutation before any other work"; "MKT-008 export -> wipe -> import-backup: a full round trip is lossless and preserves each row's ORIGINAL provider (never relabelled)"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `/api/market-data/price-uploads/:batchId` (MKT-008, `deletePriceUploadAction`)                                                                          | DELETE        | `getAuthenticatedSqlContext()` → `context.userId`; `deletePriceUploadBatch` re-verifies `WHERE id = ? AND user_id = ?` before any delete, both for the batch row and its `price_observations` (`upload_batch_id = ?`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Yes (new, `app/api/market-data/price-uploads/[batchId]/route.ts`)                                                            | `tests/mkt-008.test.ts` "MKT-008: every mutating price-upload route calls rejectCrossSiteMutation before any other work"; "MKT-008 delete: removes exactly the upload's own rows and the batch row, owner-scoped; re-importing the same file restores identical facts"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
