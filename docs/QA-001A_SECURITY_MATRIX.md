...
→ **9** as of UI-006C (8 at UI-006B time, +1:
...
path exposing 1 new handler (the refresh route's `POST`), so the current,
single source of truth is **30 API route paths exposing 34 exported
HTTP-method handlers, plus 9 pages**.
...
`app/dividend-assumptions-actions.ts` and UI-006C's
`app/owned-security-dividends.ts`/`app/dividend-history-refresh-actions.ts`
do not match this `*route.ts` suffix pattern, so none of them add a new
non-route match). 30 real route files + 5 non-route modules = 35, the count
this task's verification run of `vinext check` reports (up from the 34
recorded at UI-006B time; 30 at QA-001A time, 25 + 5).
