...
**Defense in depth with the guest account:** BRK-002 also has the app authenticate as a separate free Sharesight account holding only a read-only share of the owner's portfolio (not the owner's own Sharesight login). Even if this module's GET-only enforcement were somehow bypassed, the guest account's own read-only share would still reject a write at Sharesight's end — two independent controls, not one.
...
**Parsing discipline — absent vs malformed:** `domain/sharesight/parse.ts`'s `optionalDecimal` distinguishes a genuinely absent/null optional money field (an honest `null` "unknown") from one that is PRESENT but fails to parse as a decimal. The latter fails the whole item closed (`invalid_response`), never silently collapsing to the same `null` "unknown" state, because several of these optional fields (franked/unfranked payout amounts, withheld tax) feed downstream tax assumptions — a silently-dropped corrupt value there would be worse than an explicit failure. Two smaller shape assumptions are marked with `TODO(BRK-008)` rather than resolved here, pending the live spike: whether Sharesight v3 IDs are numeric or string, and whether decimal-from-number conversion can ever produce exponential notation for a realistic money/quantity magnitude.
