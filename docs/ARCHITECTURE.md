...
- **Trades.** `id` is numeric (like portfolios/holdings, not the string BRK-003 assumed). REQUIRED: `id`, `transaction_date`, `quantity`, `price`, `holding_id`, `portfolio_id` (both numeric, normalized the same way as `id`). The trade's own `portfolio_id` is validated for presence, shape, AND equality against the caller-supplied `portfolioId` this fetch was scoped to — a mismatch fails the whole item closed (envelope fail-close convention), never silently re-attributing a mis-scoped record to the queried portfolio, which would otherwise be a provenance lie downstream. `value` is nullable-tolerant (an optional decimal, not required) since other fields on the same live item (`paid_on`, `market_price`) are confirmed null, and a hard requirement risked failing real trades closed unnecessarily. `transaction_type` is now OPTIONAL — the live item's first record did not carry it at all; absent is an honest `null`, while a present-but-unrecognized value (outside `buy`/`sell`/`other`) still fails the item closed (absent-vs-malformed discipline). New optional fields modelled: `brokerage_currency_code`, `exchange_rate`(_pair), `state`, `unique_identifier`, `paid_on`, `description_code`, `source_category`. `BRK-005` scoping note: `quantityDecimal` is parsed non-negative (`allowNegative: false`, unchanged from BRK-003), so an absent `transaction_type` leaves NO recoverable buy/sell direction for that trade from this field alone — quantity's sign cannot substitute for it. Combined with holdings' own missing quantity above, `BRK-005` must decide how a trade with a null `transactionType` is treated (e.g. excluded from position reconstruction, or resolved via `description_code`/another signal) before ingestion can rely on this endpoint alone for position/direction.
- **Payouts endpoint path corrected.** The live `listPayouts` call returned `invalid_response` with **no** shape diagnostic — a body that was not valid JSON at all (the un-suffixed `/portfolios/:id/payouts` path), unlike the `.json`-suffix-free `holdings`/`trades` v3-native routes, which worked as-is. Sharesight's v3 API documents the portfolio-payouts endpoint as `/portfolios/:portfolio_id/payouts.json` (confirmed against the provider's published Swagger-derived client); `client.ts` now requests that exact path. The payout field contract itself (`SharesightPayout`) is deliberately left AS-IS pending confirmation against a real response on the corrected path — it will either parse or, if the assumed shape is wrong, fail closed with a shape diagnostic on the next run, which is the intended outcome either way.
- **New diagnostic: `onBodyParseDiagnostic`.** The `listPayouts` symptom above — a body read successfully but not valid JSON at all — is a distinct failure class from a parsed-but-invalid domain shape, and previously surfaced with zero diagnostic evidence (`onShapeEvidence` only fires when there IS parsed JSON to derive a shape from). `client.ts` now exposes a sibling `onBodyParseDiagnostic` option, fired only from that one `JSON.parse` failure branch, carrying metadata only — `contentType`, `httpStatus`, the fixed `bodyParseable: false` marker, and a byte count — never the body itself, matching `onShapeEvidence`'s no-values discipline. Wired into `sharesight-read-spike.mjs` alongside the existing shape diagnostic.
