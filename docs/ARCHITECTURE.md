...
- **Payouts endpoint path corrected.** The live `listPayouts` call returned `invalid_response` with **no** shape diagnostic. At the time, this was attributed to a body that was not valid JSON at all (the un-suffixed `/portfolios/:id/payouts` path returning something other than JSON) — **that attribution was an INFERENCE, not a directly observed fact**: no diagnostic of any kind fired at the time, so nothing actually confirmed WHY the response failed to parse (see the 2026-08-15 follow-up below, which found a second, equally-plausible mechanism that produces the identical symptom). Sharesight's v3 API documents the portfolio-payouts endpoint as `/portfolios/:portfolio_id/payouts.json` (confirmed against the provider's published Swagger-derived client); `client.ts` now requests that exact path regardless of which mechanism actually caused the original failure. The payout field contract itself (`SharesightPayout`) is deliberately left AS-IS pending confirmation against a real response on the corrected path — it will either parse or, if the assumed shape is wrong, fail closed with a shape diagnostic on the next run, which is the intended outcome either way.
