...
| `SHARESIGHT_PAYOUT_KEY_COLLISION` | error    | two or more Sharesight payouts in one fetch share the SAME identity key (holding + paid_on) -- all are staged for visibility, never auto-disambiguated, and this batch (plus every future sync's own batch) is permanently blocked from readiness until the duplicate is resolved inside Sharesight itself and re-synced (BRK-005C review-round correction) |
