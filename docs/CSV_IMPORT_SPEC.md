...
**Withdrawal.** After every observed payout in this fetch is upserted, `markWithdrawnNotObserved` withdraws every previously-active row for this portfolio that was NOT observed this time AND that the payout stream's own fetch window (`SharesightSyncWindow.payouts`) actually covered -- a `"full"` window withdraws any not-observed row, a `"narrowed"` one only a row whose `ex_date` falls on or after the window's `sinceDate` (a null-`ex_date` row is only ever withdrawn by a full window, mirroring the routine sync's own honesty rule elsewhere). Ordering note: a partial multi-chunk upsert failure is safe for withdrawal because the observed set the withdrawal step receives is always a superset of what was actually written -- but this codebase's `upsertObserved` fails a batch WHOLESALE on any chunk failure (never partially), so in practice withdrawal simply does not run at all when the upsert itself failed (`pendingPayoutsError` set instead).
...
**Panel copy.** `app/sharesight-sync-panel-helpers.ts`'s `formatSyncResultMessage` appends one honest, omitted-at-zero line naming how many announced dividends were recorded, with a parenthetical count of any that could not be matched to a holding, or -- taking priority over the count -- the honest error message when recording itself failed.
