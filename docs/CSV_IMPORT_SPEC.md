...
**BRK-022 slice 2 (2026-09-04): future-dated null-id payouts are now RECORDED, not thrown away.** `isFutureUnconfirmedPayout` (`domain/sharesight-sync/transform.ts`, exported unchanged for slice 2's reuse) still skips a future-dated (not-yet-due) null-id payout from THIS batch -- there is still no reliable "this really happened" fact to stage as a transaction/dividend row -- but `app/sharesight-sync-service.ts` now records every such payout as its own row in `sharesight_pending_payouts` (a separate OBSERVATION table, `docs/DATA_MODEL.md`), upserted directly at sync time with no staging/review step of its own (an announcement is a provider observation, not a ledger fact). `sharesight_pending_payouts.source_reference` stores the BARE `payoutIdentityKey` (also exported), the same value the staged/committed path's row `fingerprint` carries -- but it is NOT byte-equal to the committed row's own `source_reference`. **Review round correction (B2, BLOCKING, 2026-09-04): an earlier version of this paragraph claimed the two were "the identical key" -- false.** `db/repositories/import-commit.ts` wraps every staged row's fingerprint in an `import-fingerprint:` prefix (shared by every import source, CSV or Sharesight) to mint a committed `source_reference`; the pending table's key is that same prefix's INPUT, not its output. `domain/imports/committed-source-reference.ts`'s `committedSourceReferenceForFingerprint(fingerprint)` is the single exported function that derives one from the other -- slice 3's "paid overrides pending" suppression check must call it on the pending row's `sourceReference` before comparing against a committed row's `source_reference`, never compare the bare and prefixed forms directly. Runs on BOTH a freshly-staged and a REUSED batch (like BRK-014's already-imported counts, it reflects the account's CURRENT Sharesight state, not a staging-time snapshot), and is best-effort: a recording failure never fails the sync itself (the batch is already safely staged); the sync result's `pendingPayoutsError` field surfaces it honestly instead.

**Resolution to a security, never a guess.** Each pending payout resolves to an EXISTING `portfolio_securities` row for this user+portfolio in two strict tiers -- (1) the Sharesight instrument id, when the payout carries one, against `security_identifiers.scheme = 'sharesight_instrument'`; (2) failing that, symbol + exchange, using the SAME `instrumentMatchKey` normalisation (trim + uppercase) `payoutSecurityCurrencyProxy`/`tradeCurrencyByInstrumentKey` already use for FX-currency evidence. A tier match that is ambiguous (more than one candidate) resolves to `null` OUTRIGHT, never falling through to the weaker tier and never guessing; `null` is stored as `portfolio_security_id`, counted (`pendingPayoutsUnresolved`), and never blocks. This NEVER creates a security -- an instrument this account has not linked yet simply stays unresolved until a later sync (once a trade or another payout resolves it).

**FX fields mirror the committed dividend path's derivation exactly (BRK-010), never the commit-time fail-closed case.** When the payout resolves to a security and its currency differs from that security's own (`isForeignToSecurity`, the identical rule `db/repositories/import-commit.ts`'s dividend branch applies), `fx_rate_to_portfolio_decimal`/`fx_rate_source` are populated from the SAME `invertToPortfolioConversionRate(payout.exchangeRateDecimal)` call the committed path uses -- `'sharesight'` provenance, or both left `null` when no usable rate was supplied. Unlike the committed path's case B (achievable conversion, no rate -> fails closed with `mapping_incomplete`), an observation is never blocking: a foreign payout with no usable rate simply stores `null`/`null`, same as a genuinely native payout or an unresolved one (no security to compare currencies against at all).

**Withdrawal.** `markWithdrawnNotObserved` withdraws every previously-active row for this portfolio that was NOT observed this time AND that the payout stream's own fetch window (`SharesightSyncWindow.payouts`) actually covered -- a `"full"` window withdraws any not-observed row, a `"narrowed"` one only a row whose `ex_date` falls on or after the window's `sinceDate` (a null-`ex_date` row is only ever withdrawn by a full window, mirroring the routine sync's own honesty rule elsewhere). **Review round correction (B1, BLOCKING, 2026-09-04): the observed set is EVERY payout `listPayouts` returned this fetch, not just the future-dated ones upserted above.** An earlier version built the observed set only from the future-dated candidates, so the very next sync AFTER a pending payout's pay date passed -- while Sharesight still lists it and nothing has committed yet -- withdrew it: it is no longer future-dated (so it is not in the candidate set), yet it is also not yet staged as a committed row (so nothing else keeps it alive). The observed set is now every payout's own `payoutIdentityKey`, confirmed or not, stageable or not -- an active pending row stays active for as long as Sharesight keeps listing that payout at all, and stops counting only once slice 3's read-time suppression sees the committed twin, or Sharesight genuinely drops it.

**Review round correction (B3, BLOCKING, 2026-09-04): "`upsertObserved` fails a batch WHOLESALE on any chunk failure (never partially)" -- an earlier version of this paragraph claimed that; it is false.** `upsertObserved`'s VALIDATION is wholesale (an `invalid_input` row means nothing in the call is written), but its writes are sent as several `client.batch()` calls, one per `REFERENCE_CHUNK_SIZE`-sized chunk of statements -- a chunk failure partway through can leave EARLIER chunks applied. This is safe because an observation is idempotent (the next sync re-observes and re-upserts every row regardless of what a prior partial call left behind), and withdrawal is skipped entirely on any `upsertObserved` failure (`pendingPayoutsError` set instead), so a partial write is never compounded by a withdrawal pass reasoning from an incomplete observed set.

**A paid-date change is a withdraw-and-recreate, not an update in place.** `payoutIdentityKey` includes `paidOnDate`, so Sharesight moving a still-unconfirmed payout's paid date produces a genuinely different key -- the OLD key is withdrawn (no longer observed) and the NEW key is recorded as a fresh pending row in the same sync.

**Colliding future-dated payouts are never recorded arbitrarily (review round F2, 2026-09-04).** Two future-dated payouts sharing one `payoutIdentityKey` in the SAME fetch (same holding, same `paid_on` -- e.g. an interim and a special dividend both still unconfirmed) cannot be told apart, exactly the collision `SHARESIGHT_PAYOUT_KEY_COLLISION` already blocks for STAGEABLE rows (`buildPayoutRow`). `app/sharesight-sync-service.ts` detects this over the future-dated set with the SAME `countPayoutKeyCollisions` helper the transform uses, and skips recording EITHER of a colliding pair -- picking one arbitrarily would silently prefer one indistinguishable announcement over the other. Neither is written to `sharesight_pending_payouts`; if a key was previously recorded (from an earlier, non-colliding sync) it is left untouched rather than withdrawn, since the observed set (see Withdrawal above) still contains it. This collision is disclosed to the owner only via the sync panel (`pendingPayoutsCollided`/`pendingPayoutsLine`), not via a persisted `import_issues` row -- a DELIBERATE choice (this recording is not part of a staged import batch at all), not because a row-less top-level `import_issues` entry is impossible to persist (`db/repositories/import-staging.ts` already does exactly that for `SHARESIGHT_PAYOUT_UNCONFIRMED`, BRK-022 slice 3 review correction). The sync result's `pendingPayoutsCollided` count discloses how many payouts this affected, and the panel appends a note naming the count when non-zero. The remedy is the same as the stageable-row collision: resolve/deduplicate the payout inside Sharesight itself, then re-sync.

**Panel copy.** `app/sharesight-sync-panel-helpers.ts`'s `formatSyncResultMessage` appends one honest, omitted-at-zero line naming how many announced dividends were recorded, with a parenthetical count of any that could not be matched to a holding, plus a second parenthetical when any were not recorded because of a same-fetch identity collision -- or, taking priority over the whole line, the honest error message when recording itself failed.
...
- **Preview time.** `domain/imports/reconciliation.ts` now runs the identical field-by-field comparison (`domain/imports/committed-value-comparison.ts`'s `tradeValueDifferences`/`dividendValueDifferences`, extracted from `BRK-014`'s own classifier so preview and sync-result can never independently drift) for any row already bound for the commit-time identity skip (§10/§13). A row whose value STILL matches stays a true no-op, silently skipped exactly as before. A row whose value DIFFERS raises a NEW error-severity, row-linked issue, `ROW_DIFFERS_FROM_COMMITTED_RECORD` (committed vs incoming values named per field, both in the message and in a structured `fieldDifferences` list). **`BRK-019` CORRECTION ROUND (2026-09-04): the sentence that followed here -- "blocks readiness for THAT ROW ONLY -- the rest of the batch stays committable" -- overstated what `preview.ready` itself does. `preview.ready` is a single BATCH-level boolean (`!issues.some(severity === "error")`): while ANY row in the batch carries this issue, the WHOLE batch is blocked from reaching `ready` (and, for a Sharesight `sharesight_sync` batch, from `acceptImportWithContext`'s atomic Accept), not just that one row's own eligibility. What DOES hold, and is the actual enforcement chain: excluding the affected row (`IMP-008`'s "Skip this row") drops it from reconciliation entirely, which flips `preview.ready` back to true and lets the REST of the batch proceed -- so the practical effect reads as "that row, once excluded, stops being the thing blocking everything else." The single AUTHORITATIVE, never-bypassable enforcement is commit's own live, per-row fail-closed skip (the next bullet), which re-derives the same finding independently of whatever the preview showed. On the Sharesight route specifically, `app/import-ready-service.ts`'s ready-time check (shared with `app/import-accept-service.ts` via `app/import-ready-review-loader.ts`) now also PERSISTS this finding into `import_issues` the moment it is detected, so the review UI's persisted-issue-driven "Accept Import" button (`app/components/import-review.tsx`'s `acceptDisabled`, unchanged by this task -- see `BRK-009C`) is visibly blocked before the owner ever clicks it, not just after a failed attempt.** **`BRK-019` ROUND 3 (2026-09-04), correcting the sentence immediately above: that claim is false. Persistence happens INSIDE `markImportReadyWithContext`, i.e. during the first Accept/Mark-ready attempt -- so the first Accept/Mark-ready attempt is rejected and persists the finding, and the button is disabled from the next render onward. It is NOT blocked before the owner ever clicks it, and slice 1 deliberately implements no pre-emptive blocking (that would require loading the page-only committed-value evidence on every render). What the persistence does buy is that the owner never gets a second, unexplained rejection: after the first attempt the block is visible, named on the row, and the button is disabled.** The SAME code also covers the `paidOnDate` case: a paid-date correction mints a NEW payout identity key (§3's BRK-005C note), so it is never an identity match at all -- but when the existing DIV-004 economic near-match (§13) fires against a committed record that is itself Sharesight-sourced (`source_reference` carrying the `sharesight-payout:` prefix) with the same portfolio, security, and cash total, this now ALSO escalates to `ROW_DIFFERS_FROM_COMMITTED_RECORD` rather than staying a plain advisory warning -- a near-match against a manually-entered or CSV record is UNCHANGED (stays a warning; two such legitimate economic-identity collisions are confirmed to exist on the owner's own account, so an automatic match is never safe there).
- **Commit time (the fail-closed backstop).** `db/repositories/import-commit.ts`'s existing exact-identity skip (trade and dividend branches alike) now independently re-derives the SAME comparison, live, against whatever is CURRENTLY committed -- never trusting the preview alone, since a stale preview, a resumed older batch, or a concurrent change could all let a value-changed row reach commit with no earlier warning. A row whose value differs is skipped exactly as any duplicate is (`commit_status = 'skipped'`, no write either way, old or new), but ALSO gets a persisted, distinctly-coded `import_issues` row (see the persistence note below) in the SAME atomic statement group as the skip, so the sync/commit summary can report it as its OWN bucket rather than folding it into an ordinary duplicate skip. **`BRK-019` CORRECTION ROUND (2026-09-04): a `paidOnDate` correction mints a NEW `source_reference`, so this exact-identity skip could never see it, and the row took the ordinary create path -- a real double-write (reviewer repro: sync, commit, re-sync with the SAME distribution's paid date shifted, Accept). Commit now ALSO runs a second, near-match live re-derivation for this shape specifically: before creating a dividend record, it looks up committed `sharesight-payout:`-sourced records for the same owner/portfolio/security within DIV-001's proximity window with a matching cash total (`safeComputeDividendCashTotal` on both sides); a match skips the row with the identical persisted `ROW_DIFFERS_FROM_COMMITTED_RECORD` issue, counted the same way in `needsDecisionRows`.** **`BRK-019` ROUND 3 (2026-09-04): that near-match candidate query is now scoped to the PRE-BATCH snapshot -- it excludes any record THIS batch itself wrote (`import_batch_id IS NULL OR import_batch_id <> <this batch>`), matching exactly what the preview-time escalation can see. `commit()` writes in chunks, so without that predicate a record written by an EARLIER chunk of the same batch counted as an "existing" record: two LEGITIMATE distributions on one security days apart at the same cash total (a shape confirmed on the owner's own account) silently lost the second one whenever the pair straddled a chunk boundary, and committed both when it did not -- a chunk-position-dependent financial outcome. A genuine paid-date correction always arrives in a LATER batch than the record it corrects, so it is unaffected and still caught. The near-match skip also now carries its OWN `import_issues` message ("looks like a paid-date correction to an existing Sharesight-sourced record..."); the exact-identity wording ("this row's identity already exists on the ledger") was factually wrong for a case whose whole premise is that the corrected paid date minted a BRAND NEW identity key.**
